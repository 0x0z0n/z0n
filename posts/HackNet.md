...
| : | : | :-- | :- |
| 1    | `N/A`                           | **Nmap / Recon**                                       | Discovered open ports: `22` (SSH) and `80` (HTTP — nginx). App identified as a Django-based social network (“HackNet - social network for hackers”).                                                                                                                                                                                         |
...
| 3    | `attacker (web)`                | **Template-context leakage (rendered `users`)**        | Controlled a username to inject `{{ users }}` (and `{{ users.values }}`) into a post/like rendering, causing internal `QuerySet`/field dicts to be displayed in the likes list.                                                                                                                                                              |
| 4    | `attacker (web)`                | **Automated scraping of leaked fields**                | Scripted likes + scrape of `<img title="...">` content to extract `'email'` and `'password'` fields from the rendered `users.values`. Collected many site credentials (user:password pairs).                                                                                                                                                 |
...
| 6    | `attacker (www-data?)`          | **Cache poisoning — Django FileBasedCache + Pickle**   | Found `@cache_page(60)` in `explore` view. Generated a malicious pickle payload (class with `__reduce__` returning `os.system(cmd)`), wrote it into `.djcache` files in `/var/tmp/django_cache`, then requested `/explore` to trigger deserialization and execute a command (reverse shell). Achieved remote code execution on the web host. |
| 7    | `sandy` (local user discovered) | **Post-exec enumeration — discover user files & keys** | After RCE, enumerated web and home directories. Found `sandy` user and discovered `armored_key.asc` (GPG private key) and encrypted backup `.gpg` files inside `/var/www/HackNet/backups`.                                                                                                                                                   |
| 8    | `attacker (with imported key)`  | **GPG private key import & bulk decrypt**              | Imported the `armored_key.asc` and used it (with the discovered passphrase hint `"I CANT TELL YOU"`) to decrypt backup `.gpg` files. Extracted SQL/backup contents and searched for plaintext credentials (e.g., database user passwords, root credentials).                                                                                 |
| 9    | `root`                          | **Credential reuse / SSH login → root**                | Located/root credentials in decrypted backups (or found an SSH-able root key) and authenticated as `root`. Retrieved `root.txt`.                                                                                                                                                                                                             |
